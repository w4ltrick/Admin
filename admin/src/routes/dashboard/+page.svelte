<script>
  ...
  let mesas = [
      ...
      { numero: 4, pedido: ["Pizza Margherita"], total: 35.00, garcom: "Juliana" }
  ];
      ...
      {:else if abaAtual === 'mesas'}
        <div class="flex flex-col">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            {#each mesas as mesa}
              <button 
                class={`p-4 rounded-lg shadow-md transition transform hover:scale-105 flex flex-col ${mesaSelecionada === mesa.numero ? 'bg-red-100 border-2 border-red-500' : 'bg-white'}`}
          ...
          {#if mesaSelecionada}
            <div class="mt-8 bg-white p-6 rounded-lg shadow-md flex flex-col">
              <div class="flex justify-between items-center mb-4">
              ...
              <div class="bg-gray-50 p-4 rounded mb-4 flex flex-col">
